...
# Load the ruby mime/types library (used by the Octokit library) unless 
...
if not defined?(MIME::Type)
  ...
  handler_path = File.expand_path(File.dirname(__FILE__))
  # Calculate the location of our library and add it to the Ruby load path
  library_path = File.join(handler_path, 'vendor/mime-types-1.23/lib')
  $:.unshift library_path
  # Require the library
  require 'mime-types'
end
...
elsif MIME::Type::VERSION.to_s != '1.23'
  raise "Incompatible library version #{MIME::Type::VERSION} for mime/types.  Expecting version 1.23."
end
